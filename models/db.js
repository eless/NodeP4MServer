...
var getTournaments = function(callback){
    var sql    = 'SELECT name, description, tournament_id, uCount FROM p4m.d2_tournaments AS tbl1 left join (select tournament_id as tournament_id, count(account_id) as uCount from d2_users_daily group by tournament_id) AS tbl2 on tbl2.tournament_id = tbl1.id where tbl1.active = 1';
    connection.query(sql, function(error, results) {
        if(error) throw error;
        callback.call(this, results);
    });
